...
On Debian, Ubuntu, Mint &c click *releases*, then *dosbox-x\_0.83.2\_i386.deb* to download it.  When the download has completed you can install it by double-clicking it or entering `sudo dpkg dosbox-x_0.83.2_i386.deb` from a terminal.  The binary itself is also in the `binary` directory above for people not using Debian-based distros.  
