...
Printing this way can be automated by use of the two scripts `dbx-print` and `CapturePrint`.  dbx-print calls CapturePrint, which actually does the printing,  then runs dosbox-x and when you quit that switches off the printing mechanism.  NOTE:  BOTH THESE SCRIPTS ARE LIKELY TO NEED AMENDING TO SUIT YOUR SET-UP.  As they stand they assume that dosbox-x sends files it has captured from LPT1 to `~/capture`, that the scripts are in `~/.dosbox` and that the dosbox-x binary is somewhere in your `$PATH`.
